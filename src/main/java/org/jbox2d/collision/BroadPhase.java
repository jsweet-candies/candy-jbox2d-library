...
public class BroadPhase {
	...
	private void dump() {
		...
		for (int i = 0; i < 100; i++) {
			System.out.println("bounds[ " + i + " ] = " + m_bounds[0][i].value + ", " + m_bounds[1][i].value + " \n");
		}

		for (int i = 0; i < m_proxyPool.length; i++) {
			Proxy p = m_proxyPool[i];
			if (p != null) {
				System.out.println(
						"proxy#" + i + ": valid=" + p.isValid() + " userData=" + " upperBounds=" + asList(p.upperBounds) //
								+ " lowerBounds=" + asList(p.lowerBounds) //
	...
	public BroadPhase(final AABB worldAABB, final PairCallback callback) {
		...
		if (debugPrint) {
			System.out.println("BroadPhase: m_quantizationFactor=" + m_quantizationFactor);
		}
	...
	public int createProxy(final AABB aabb, // int groupIndex, int categoryBits, int
		...
		if (BroadPhase.debugPrint) {
			System.out.println("CreateProxy() bbox=" + aabb + " proxyId=" + proxyId + "/" + m_proxyCount
					+ " m_freeProxy=" + m_freeProxy);
		}
		...
		for (int axis = 0; axis < 2; ++axis) {
			...
			System.arraycopy(m_bounds[axis], upperIndex, m_bounds[axis], upperIndex + 2, boundCount - upperIndex);
			...
			System.arraycopy(m_bounds[axis], lowerIndex, m_bounds[axis], lowerIndex + 1, upperIndex - lowerIndex);
	...
	public void destroyProxy(final int proxyId) {
		...
		if (BroadPhase.debugPrint) {
			System.out.println("destroyProxy(" + proxyId + ")");
			dump();
		}
		...
		for (int axis = 0; axis < 2; ++axis) {
			...
			// memmove(bounds[lowerIndex + 1], bounds[lowerIndex],
			// (upperIndex - lowerIndex) * sizeof(b2Bound));
			System.arraycopy(m_bounds[axis], lowerIndex + 1, m_bounds[axis], lowerIndex, upperIndex - lowerIndex - 1);
	...
	public void moveProxy(final int proxyId, final AABB aabb) {
		if (BroadPhase.debugPrint) {
			System.out.println("MoveProxy(" + proxyId + "/" + m_proxyCount + ")");
		}
		...
		for (int axis = 0; axis < 2; ++axis) {
			if (m_bounds[axis][proxy.lowerBounds[axis]] != null) {
		...
		for (int axis = 0; axis < 2; ++axis) {
			...
			final int deltaLower = bounds[lowerIndex] == null ? 0 : lowerValue - bounds[lowerIndex].value;
			final int deltaUpper = bounds[upperIndex] == null ? 0 : upperValue - bounds[upperIndex].value;
	...
	/**
	 * Query an AABB for overlapping proxies, returns the user data and the count,
	 * up to the supplied maximum count.
	 */
	...
	private void computeBounds(final Integer[] lowerValues, final Integer[] upperValues, final AABB aabb) {
		...
		final float bx = aabb.lowerBound.x < m_worldAABB.upperBound.x ? aabb.lowerBound.x : m_worldAABB.upperBound.x;
		final float by = aabb.lowerBound.y < m_worldAABB.upperBound.y ? aabb.lowerBound.y : m_worldAABB.upperBound.y;
		...
		final float b1x = aabb.upperBound.x < m_worldAABB.upperBound.x ? aabb.upperBound.x : m_worldAABB.upperBound.x;
		final float b1y = aabb.upperBound.y < m_worldAABB.upperBound.y ? aabb.upperBound.y : m_worldAABB.upperBound.y;
		...
		if (BroadPhase.debugPrint) {
			System.out.println("ComputeBounds() vals: lowerValues[0]=" + lowerValues[0] + " upperValues[0]="
					+ upperValues[0] + " lowerValues[1]=" + lowerValues[1] + " upperValues[1]=" + upperValues[1]
					+ " test=" + ((int) (m_quantizationFactor.x * (minVertexX - m_worldAABB.lowerBound.x))));
		}
	}
	...
	public boolean inRange(final AABB aabb) {
		...
		boolean inRange = (MathUtils.max(dx, dy) < 0.0f);

		if (debugPrint) {
			System.out.println("inRange (" + inRange + "): aabb=" + aabb + " // world[" + m_worldAABB + "]" + " => dx="
					+ dx + " dy=" + dy);
		}
	...
	public int querySegment(Segment segment, Object[] userData, int maxCount, SortKeyFunc sortKey) {
		...
		// If we are using sortKey, then sort what we have so far, filtering negative
		// keys
		if (sortKey != null) {
			// Fill keys
			for (int i = 0; i < m_queryResultCount; i++) {
		...
		for (;;) {
			...
			for (;;) {
				if (sy == 0 || (sx != 0 && xProgress < yProgress)) {
					...
					// Early out
					if (sortKey != null && m_queryResultCount == maxCount && m_queryResultCount > 0
							&& xProgress > m_querySortKeys[m_queryResultCount - 1])
						break;
					...
					// Early out
					if (sortKey != null && m_queryResultCount == maxCount && m_queryResultCount > 0
							&& yProgress > m_querySortKeys[m_queryResultCount - 1])
						break;
	...
	private void addProxyResult(int proxyId, Proxy proxy, int maxCount, SortKeyFunc sortKey) {
		...
		// float32* p =
		// std::lower_bound(m_querySortKeys,m_querySortKeys+m_queryResultCount,key);
		int i = 0;
